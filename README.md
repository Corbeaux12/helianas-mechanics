...
| Heliana's Hunt Items | Item (dnd5e) | Items transcribed from the Loot Tavern hunt PDFs (`tools/data/hunts/*.json`): craftables (one per rarity tier, full description/rarity/attunement/price), harvested components, and a "Hunt Notes" recipe-book item per hunt. Pilot: **Cracker Kraken** (Calamarmour, Hyper Soaker, Torrential Mantle × 3 tiers). See [docs/hunts-plan.md](docs/hunts-plan.md) for the import roadmap. |
