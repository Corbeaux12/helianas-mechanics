...
| Heliana's Hunt Items | Item (dnd5e) | Items transcribed from the Loot Tavern hunt PDFs (`tools/data/hunts/*.json`): craftables (one per rarity tier, full description/rarity/attunement/price), harvested components, and a "Hunt Notes" recipe-book item per hunt. Imported so far: Cracker Kraken, Jörmungummdr, Adelifae & Scarab, Bloodhound, Laceleaf Mantid (91 documents). See [docs/hunts-plan.md](docs/hunts-plan.md) for the import roadmap. |
| Heliana's Hunt Bestiary | Actor (dnd5e) | Boss NPCs transcribed from the Loot Tavern **Field Notes** PDFs (`tools/data/actors/*.json`): full stat block (abilities/saves/skills, AC, HP formula, speeds, senses, immunities/resistances, CR), portrait + token from the bundled `_Art` renders, and every trait/action/bonus action/reaction as a feature item with its complete rules text. Players default to **Limited** visibility. Attack-roll automation is a planned later pass. |
