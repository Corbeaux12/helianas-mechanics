...
| Storm Dragonray (The Synaptick Shiver) | ✅ 3 items (9 tiers)¹⁸ | ✅ | ✅ | ✅ | ⬜ |
| Tomb Tyrant (Tomb of the Timeless Tyrant) | ✅ 4 items (11 tiers)¹⁹ | ✅ | ✅ | ✅ | ⬜ |
| Briar Lord (Field Notes X-38) | n/a | n/a | n/a | ⬜ | ✅ pilot actor |
| Cold Blood³ · Chaos at the Coral Court³ · End Times | ⬜ texts extracted, awaiting transcription | ⬜ | ⬜ | ⬜ | ⬜ |
...
¹⁸ Clean hunt, no table-vs-description conflict (first hunt in a while where both matched cleanly on all three items). Pouch of Dragon Scales is generic; Dragonray Psitick and Dragonray Barbed Tail are hunt-specific (not in the harvesting index), priced by harvest DC (Psitick DC 20 → 110 gp, Barbed Tail DC 10 → 40 gp). The adventure's "Wingsuit" is mundane adventuring gear with a manufacture DC placeholder, not listed in the hunt's own Craftable Items table, so it was skipped rather than treated as a magic item.
¹⁹ A new variant of the table-vs-description conflict: the tomb tyrant's Harvest Table lists both a common "Eye" (DC 5, ×10 available) and a separate rare "Main Eye" (DC 20, ×1 available, the adventure's own hook item). The crafting table lists Sandblight Sceptres' component as plain "Eye", but the item's own description says "Main Eye" — trusted the description here since Sandblight Sceptres is the hunt's highest-rarity item (Rare/Very Rare/Legendary) and thematically fits the unique, singular Main Eye rather than the common one; Tyrant's Gaze (Uncommon/Rare only) uses the common, already-generic "Undead Eye" with no conflict. Tomb Tyrant Wrappings and Main Eye are hunt-specific components (Wrappings DC 10 → 40 gp, Main Eye DC 20 → 110 gp).
