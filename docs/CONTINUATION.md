...
- **Hunt items: 26 of 27 hunts imported** (`tools/data/hunts/*.json`). Latest: Cold Blood on the Scorching Sand (Kaftar Matriarch) — 7 items, 22 recipe pages, all verified linked. First of the two manual-carve hunts: its `.craft.txt` said "NO CRAFTING SECTION FOUND" (auto-carve failed entirely) — found the real crafting section by grepping the full `.txt` for "Harvest"/"Crafting" and reading around line 2182 by hand. Only 3 items are the boss's own printed-price table items (Cacklecloak, Kaftar's Kiss, Maw of Oumdabaa); the other 4 (Cheese Stick, Parabox, Moon Oil, Moonscreen) are unrelated appendix treasure items with no printed prices — estimated and footnoted, following the established "include appendix bonus items" pattern from earlier hunts (Krampus' Fork, Nature's Vengeance, etc.). Only "Kaftar Matriarch Tongue" is hunt-specific; everything else (Pelt, Teeth, Aberration Bone, Plant Spores, Plant Sap) is generic.
- **Only Chaos at the Coral Court remains** — same manual-carve situation expected (check for "NO CRAFTING SECTION FOUND" in its `.craft.txt` first, then grep the full `.txt` for "Harvest"/"Crafting" to find the real section by hand, same approach used for Cold Blood above). After this one, all hunt-items work is done and the "Backlog after hunts" section becomes the active task list.
...
1. All standard `*.craft.txt` hunts are done. **Only Chaos at the Coral Court remains, and it needs manual carving** — its `.craft.txt` will likely say "NO CRAFTING SECTION FOUND" (that's what Cold Blood's said); grep the full `.txt` for "Harvest"/"Crafting" and read the real section by hand instead.
