...
- **Module version:** 1.12.0 deployed. Everything through commit `488b23f` is live on the server.
...
- **Hunt items: 15 of 27 hunts imported** (`tools/data/hunts/*.json`). Latest: Spectres of Midwinter (The Krampus) — 5 items, 16 recipe pages, all verified linked. Same crafting-table-vs-description component conflict as Queen of Shadow and Thorn (see gotcha below) — this is now the SECOND hunt with this pattern, so treat it as a recurring thing to check for, not a one-off.
...
At least two hunts so far (Queen of Shadow and Thorn, Spectres of Midwinter) have a printed "Crafting" summary table listing one set of components, while the individual item description's own "Component:" line names something completely different and unrelated (e.g., table says "Krampus' marrow", description says "Fey heart"). **Always cross-check both against the printed Harvest Table** — whichever name actually appears in the Harvest Table is ground truth; the other is OCR reflow from an unrelated sidebar/template block. Do this check on every future hunt before finalizing components.

## The per-batch workflow (repeat until done)

1. Pick next `*.craft.txt` from the scratchpad (see below) — work APL-ascending. Remaining: Timekeeper's Trials, Just Desserts, Coral Hive, Throne of the Devourer, Den of the Deceiver, Forge of the Divine Machine, Dragonfruit, Synaptic Shiver, Timeless Tyrant, End Times. **Cold Blood on the Scorching Sand + Chaos at the Coral Court need manual carving** (different layout — carve from the full `.txt` next to the `.craft.txt`).
