...
- **Hunt items: 25 of 27 hunts imported** (`tools/data/hunts/*.json`). Latest and LAST of the standard multi-level hunts: End Times (Paradox Dragon) — 8 items (the biggest hunt yet), 22 recipe pages, all verified linked, no conflicts. Five of eight components generic (Dragon Bone/Horn/Heart, Phial of Dragon Blood, Aberration Heart, Aberration Hide); "Pouch of Dragon Feathers" is hunt-specific since no Dragon-flavor pouch exists generically. Reused the "Construct Gears" name from Forge of the Divine Machine for Problem-Solver — since hunt-specific components are resolved per-hunt (not shared), this creates a second separate "Construct Gears" item rather than reusing the original; harmless but worth knowing. Chronocloak/Tick & Tock/Timelord Armour form a "Timelord's Regalia" set bonus — duplicated the shared set text into all three descriptions since there's no set-item concept in the schema.
- **Only Cold Blood on the Scorching Sand and Chaos at the Coral Court remain**, both flagged as needing manual carving (different PDF layout than the standard `.craft.txt` — see workflow step 1). After those two, all hunt-items work is done and the "Backlog after hunts" section becomes the active task list.
...
1. All standard `*.craft.txt` hunts are done. **Only Cold Blood on the Scorching Sand + Chaos at the Coral Court remain, and both need manual carving** (different layout than the other hunts — carve from the full `.txt` next to the `.craft.txt`, no pre-cut crafting section to rely on).
